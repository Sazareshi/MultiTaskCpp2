...
void CAnalyst::update_as_ctrl() {
	...
	if (pIO_Table->ref.b_slew_manual_ctrl) {
		...
		(pIO_Table->as_ctrl.tgD_abs[AS_SLEW_ID] < g_spec.as_compl_nposLv[I_AS_LV_COMPLE])) {	//振れが完了判定内
		pMode->antisway_ptn_t = AS_PTN_0;
		pMode->antisway_control_t = AS_MOVE_COMPLETE;
	}
	else if (pIO_Table->as_ctrl.tgD_abs[AS_SLEW_ID] < g_spec.as_compl_nposLv[I_AS_LV_TRIGGER]) {	//目標が位置決め起動判定距離内
		if (pIO_Table->physics.sway_amp_t_ph > g_spec.as_compl_swayLv_sq[I_AS_LV_TRIGGER]) {	//振れがトリガ判定値以上
	...
	else if (pIO_Table->physics.sway_amp_t_ph > g_spec.as_compl_swayLv_sq[I_AS_LV_DAMPING]) { //振れがダンピング判定値以上
		pMode->antisway_control_t = AS_MOVE_ANTISWAY;
		pMode->antisway_ptn_t = AS_PTN_DMP;
	}
