...
extern CMODE_Table*	pMode;				//ã§óLÉÅÉÇÉäModeÉNÉâÉXÉ|ÉCÉìÉ^
extern CORDER_Table*pOrder;			//ã§óLÉÅÉÇÉäOrderÉNÉâÉXÉ|ÉCÉìÉ^
extern ST_SPEC		g_spec;			//ÉNÉåÅ[Éìédól
...
void CPlayer::routine_work(void *param) {
	...
	update_auto_status();			//é©ìÆêUé~èÛë‘çXêV
	cal_auto_ref();					//PCåvéZ ë¨ìxéwóﬂåvéZ
	set_table_out();				//èoóÕÉZÉbÉg
...
int CPlayer::set_table_out() {
	...
	if(manual_vref[MOTION_ID_BH] != 0.0) 
		pIO_Table->ref.b_bh_manual_ctrl= TRUE;		//éËìÆëÄçÏíÜ
	else if(pIO_Table->physics.vR == 0.0)
		pIO_Table->ref.b_bh_manual_ctrl = FALSE;	//éËìÆëÄçÏíÜâèú
	else;

	if (manual_vref[MOTION_ID_SLEW] != 0.0)
		pIO_Table->ref.b_slew_manual_ctrl = TRUE;	//éËìÆëÄçÏíÜ
	else if (pIO_Table->physics.wth == 0.0)
		pIO_Table->ref.b_slew_manual_ctrl = FALSE;	//éËìÆëÄçÏíÜâèú
	else;

	if (manual_vref[MOTION_ID_MH] != 0.0)
		pIO_Table->ref.b_mh_manual_ctrl = TRUE;		//éËìÆëÄçÏíÜ
	else if (pIO_Table->physics.vL == 0.0)
		pIO_Table->ref.b_mh_manual_ctrl = FALSE;	//éËìÆëÄçÏíÜâèú
	else;
...
int CPlayer::set_motion_receipe() {
	...
	//### normal direction
	if((pMode->antisway == OPE_MODE_AS_ON) //é©ìÆâ¬ÉÇÅ[ÉhÇ≈ÉpÉ^Å[ÉìÇÃèÄîıäÆóπ
	...
	{
#if 0
		p_motion_ptn[SLW_AXIS] = &(pOrder->job_A.command_recipe[pOrder->job_A.job_step_now].motions[SLW_AXIS]);//ÉåÉVÉsç\ë¢ëÃÉZÉbÉg
		p_motion_ptn[BH_AXIS] = &(pOrder->job_A.command_recipe[pOrder->job_A.job_step_now].motions[BH_AXIS]);//ÉåÉVÉsç\ë¢ëÃÉZÉbÉg
		pAna->cal_auto_target(CAL_FOR_AUTO_JOB_A);

		//Set pattern recipe
		//Judge Positioning or Damping
		double check_d = pIO_Table->physics.T * g_spec.bh_notch_spd[1] //1ÉmÉbÉ`àÍé¸ä˙Å@Å{Å@êUÇÍé~Çﬂ1âÒÇÃà⁄ìÆãóó£
			+ g_spec.bh_acc[FWD_ACC] * pIO_Table->auto_ctrl.as_gain_damp[AS_BH_ID] * pIO_Table->auto_ctrl.as_gain_damp[AS_BH_ID];
		double check_d2 = pIO_Table->physics.T * g_spec.bh_notch_spd[1] / 3.0 //1ÉmÉbÉ` 1/3é¸ä˙Å@Å{Å@1ÉmÉbÉ`ÉCÉìÉ`ÉìÉOãóó£
		...
		if ((p_motion_ptn[SLW_AXIS]->ptn_status == PTN_UNIT_FIN) || (p_motion_ptn[SLW_AXIS]->ptn_status == PTN_NOTHING)) {//Any pattern not running
			...
			p_motion_ptn[SLW_AXIS] = &motion_ptn[SLW_AXIS];//ÉåÉVÉsç\ë¢ëÃÉZÉbÉg
		...
		if ((p_motion_ptn[MH_AXIS]->ptn_status == PTN_UNIT_FIN) || (p_motion_ptn[MH_AXIS]->ptn_status == PTN_NOTHING)) {//Any pattern not running
			...
			p_motion_ptn[MH_AXIS] = &motion_ptn[MH_AXIS];//ÉåÉVÉsç\ë¢ëÃÉZÉbÉg
...
int CPlayer::check_step_status_slew(LPST_MOTION_ELEMENT pStep) {
	...
	case CTR_TYPE_TIME_WAIT_2PN: {
		if (pStep->opt_i2 == AS_INIT_SWAY_SMALL) {
			if (pStep->act_counter > (pStep->time_count - pStep->opt_i1)) {//èâä˙êUÇÍï™í‚é~éûä‘íZèk
				status = STEP_FIN;
				if (pIO_Table->auto_ctrl.as_out_dir[AS_SLEW_ID] == AS_DIR_PLUS) {
					pIO_Table->auto_ctrl.as_out_dir[AS_SLEW_ID] = AS_DIR_MINUS;//à⁄ìÆï˚å¸êÿë÷
				}
				else {
			...
			if (pIO_Table->auto_ctrl.as_out_dir[AS_SLEW_ID] == AS_DIR_PLUS) {
				if (pIO_Table->auto_ctrl.as_start_ph[AS_SLEW_ID] == AS_START_LOW_PHASE) {
					if (pStep->act_counter > (pStep->time_count + pStep->opt_i1)) {//èâä˙êUÇÍï™í‚é~éûä‘âÑí∑
						status = STEP_FIN;
						pIO_Table->auto_ctrl.as_out_dir[AS_SLEW_ID] = AS_DIR_MINUS;//à⁄ìÆï˚å¸êÿë÷
					}
				}
				...
				if (pIO_Table->auto_ctrl.as_start_ph[AS_SLEW_ID] == AS_START_LOW_PHASE) {
					if (pStep->act_counter > (pStep->time_count - pStep->opt_i1)) {//èâä˙êUÇÍï™í‚é~éûä‘íZèk
						status = STEP_FIN;
						pIO_Table->auto_ctrl.as_out_dir[AS_SLEW_ID] = AS_DIR_PLUS;//à⁄ìÆï˚å¸êÿë÷
					}
				}
				else if (pIO_Table->auto_ctrl.as_start_ph[AS_SLEW_ID] == AS_START_HIGH_PHASE) {
					if (pStep->act_counter > (pStep->time_count + pStep->opt_i1)) {//èâä˙êUÇÍï™í‚é~éûä‘âÑí∑
					...
					if (pStep->act_counter > pStep->time_count) {//èâä˙êUÇÍï™í‚é~éûä‘íZèk
						status = STEP_FIN;
						pIO_Table->auto_ctrl.as_out_dir[AS_SLEW_ID] = AS_DIR_PLUS;//à⁄ìÆï˚å¸êÿë÷
					}
				}
	...
	case CTR_TYPE_ACC_AS_2PN: {
		if (pStep->opt_i2 == pIO_Table->auto_ctrl.as_out_dir[AS_SLEW_ID]) { //à íuçáÇÌÇπï‚ê≥ï˚å¸Ç÷ÇÃâ¡ë¨éû
	...
	case CTR_TYPE_DEC_AS_2PN: {
		if (pStep->_v < 0.0) {
			...
			if (abs(pIO_Table->physics.wth) < g_spec.slew_notch_spd[1] * 0.05) {//ê˘âÒîªíËÇÕà¯çûÇÃ1/10
				status = STEP_FIN;
			}
		}
		if (pStep->act_counter > pStep->time_count) {
...
int CPlayer::check_step_status_bh(LPST_MOTION_ELEMENT pStep) {
		...
		if (pStep->opt_i2 == AS_INIT_SWAY_SMALL) {
			if (pStep->act_counter > (pStep->time_count - pStep->opt_i1)) {//èâä˙êUÇÍï™í‚é~éûä‘íZèk
				...
				if (pIO_Table->auto_ctrl.as_out_dir[AS_BH_ID] == AS_DIR_PLUS) {
					pIO_Table->auto_ctrl.as_out_dir[AS_BH_ID] = AS_DIR_MINUS;//à⁄ìÆï˚å¸êÿë÷
				}
				else {
			...
			if (pIO_Table->auto_ctrl.as_out_dir[AS_BH_ID] == AS_DIR_PLUS) {
				...
				if (pIO_Table->auto_ctrl.as_start_ph[AS_BH_ID] == AS_START_LOW_PHASE) {
					if (pStep->act_counter > (pStep->time_count - pStep->opt_i1)) {//èâä˙êUÇÍï™í‚é~éûä‘íZèk
						status = STEP_FIN;
						pIO_Table->auto_ctrl.as_out_dir[AS_BH_ID] = AS_DIR_PLUS;//à⁄ìÆï˚å¸êÿë÷
					}
				}
				else if (pIO_Table->auto_ctrl.as_start_ph[AS_BH_ID] == AS_START_HIGH_PHASE) {
					if (pStep->act_counter > (pStep->time_count + pStep->opt_i1)) {//èâä˙êUÇÍï™í‚é~éûä‘âÑí∑
	...
	case CTR_TYPE_ACC_AS_2PN: {
		if (pStep->opt_i2 == pIO_Table->auto_ctrl.as_out_dir[AS_BH_ID]) { //à íuçáÇÌÇπï‚ê≥ï˚å¸Ç÷ÇÃâ¡ë¨éû
			if (pStep->act_counter > pStep->time_count - pStep->opt_i1) {
				status = STEP_FIN;
			}
		}
